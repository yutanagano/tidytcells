...
def standardise_template(
    ...
    # Type errors
    if type(gene) != str:
        raise TypeError("gene_name must be type str, got " f"{gene} ({type(gene)}).")
    if type(species) != str:
        raise TypeError(
            "species must be type str, got " f"{species} ({type(species)})."
        )
    ...
    if type(precision) != str:
        raise TypeError(
            "precision must be type str, got " f"{precision} ({type(precision)})."
        )
